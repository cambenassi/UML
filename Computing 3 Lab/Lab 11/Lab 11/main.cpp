// Group 10 Lab 11 -  Cam Benassi, Charles Baxter, Tri Dinh, Yicheng Huang
...
//    createUMLPeopleDB(); // not needed for Lab 11 - output is already in file lab11_input.txt
    ...
    // *** You write it!
  vector<UMLPerson>::iterator begin = upDB.begin();
  vector<UMLPerson>::iterator end = upDB.end();
  ...
  stable_sort(begin, end, compare_IDs);
  ...
  it = find(begin, end, 87654321);
  if( it != end)
    cout << "87654321 not found in UMLPerson\n";
  else
    cout << "87654321 found in UMLPerson\n";

    // Trying out STL algorithm minmax_element...
    cout << "std::minmax_element in upDB using default comparison (by last name):\n";
    // *** You write it!
  auto result1 = minmax_element(begin, end);
  cout<<"min is" <<*result1.first;
  cout<<"max is" <<*result1.second;
  cout << endl;

    cout << "std::minmax_element in upDB using Compare function compare_IDs:\n";
    // *** You write it!
  auto result2 = minmax_element(begin, end, compare_IDs);
  ...
  cout << "Using std::for_each to print full name of each person in upDB:\n";
    for_each(begin, end, printFullName);
