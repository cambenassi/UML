...
// tests bad constructor
BOOST_AUTO_TEST_CASE(badConstructor) {
    BOOST_REQUIRE_THROW(CircularBuffer test(-4), std::invalid_argument);
}
...
BOOST_AUTO_TEST_CASE(goodConstructor) {
    BOOST_REQUIRE_NO_THROW(CircularBuffer test(3));
}
