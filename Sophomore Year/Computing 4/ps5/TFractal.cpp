...
void fTree(std::vector<Triangle> &triangleVec, sf::RenderWindow &window, int &depth, double &length) {  // NOLINT
    if (triangleVec.empty()) {
        ...
        for (int i = vecSize; i >= vecSize - 3; i--) {
            double centerLeftX, centerLeftY,
            centerRightX, centerRightY,
            centerDownX, centerDownY;

            centerLeftX = (triangleVec[i].centerX - length / 2.0);
            centerLeftY = (triangleVec[i].centerY - ((sqrt(3.0) / 2.0) * length) * 2 / 3);  //NOLINT

            centerRightX = (triangleVec[i].centerX + length * 3.0 / 4.0);
            centerRightY = (triangleVec[i].centerY - ((sqrt(3.0) / 2.0) * length) * 1 / 6);  //NOLINT

            centerDownX = (triangleVec[i].centerX - length * 1.0 / 4.0);
            centerDownY = (triangleVec[i].centerY + ((sqrt(3.0) / 2.0) * length) * 5 / 6);  //NOLINT

            Triangle tempTriangleLeft(length / 2, centerLeftX, centerLeftY);
            Triangle tempTriangleRight(length / 2, centerRightX, centerRightY);
